...
def load_cifar100(args):
    ...
    if os.path.exists(f'plabels/partialY_{args.dataset}_{args.seed}.pt'):
        ...
        partialY_matrix = generate_instancedependent_candidate_labels(model.module, ori_data, ori_labels, args.rate)
...
def cifar100_partialize(images, given_partial_label_matrix, true_labels, images_diff, diff_targets): 
...
def load_cifar100_diff(args, diff_batch_size=8):
    test_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761))
    ])
    ...
    original_train = dsets.CIFAR100(root='./data/CIFAR100', train=True, download=True, transform=None)
    ori_data, ori_labels = original_train.data, torch.Tensor(original_train.targets).long()
    ...
    if os.path.exists(f'plabels/partialY_{args.dataset}_{args.seed}.pt'):
        ...
        torch.save(partialY_matrix, f'plabels/partialY_{args.dataset}_{args.seed}.pt')
    ori_data = original_train.data
    print(f'Total diffuse: {partialY_matrix.sum()}')
