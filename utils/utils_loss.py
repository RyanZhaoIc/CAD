...
class ConLoss(nn.Module):
    ...
    def forward(self, features, dist, partY, args,epoch,mask=None, batch_size=-1):
        if mask is not None:
            ...
            # compute weight
            weight = mask / (mask.sum(dim=1, keepdim=True) + 1e-12)
...
def Sym_CE_loss(probs, confidence, partY):
    partY_ = partY.detach().clone()
    probs = (1 - partY_.detach()) + (2 * partY_.detach() - 1) * probs
    loss = confidence * torch.log(probs + 1e-20)
